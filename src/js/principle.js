$(document).ready(() =>{
    let text = new Map()
    text.set("honest", "В основе нашей работы лежит прозрачность и ответственность. Мы всегда говорим о реальных сроках, возможных рисках и предлагаем оптимальные решения под конкретные задачи.");
    text.set("people", "Каждый проект делают люди для людей. Мы ценим как членов нашей команды, так и конечных пользователей наших продуктов, создавая решения, которые действительно улучшают работу.");
    text.set("technologie", "Мы используем оптимальные технологии для каждой задачи. Не гонимся за модными фреймворками, а выбираем инструменты, которые обеспечат надежность, производительность и удобство поддержки.");
    ...
    $('#honest').on('click', () => {toggle('first', 'thri', 'second')})
    $('#people').on('click', () => {toggle('second', 'first', 'thri')})
    $('#technologie').on('click', () => {toggle('thri', 'second', 'first')})
